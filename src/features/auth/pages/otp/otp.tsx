...
const OTP: React.FC<OTPProps> = ({ onConfirm, onResend }) => {
  ...
  const handleConfirmClick = async () => {
    ...
    setIsLoading(true);
    const success = await onConfirm(otpValue);
    setIsLoading(false);
    ...
    <>
      ...
      >
        <div className="bg-white shadow-lg rounded-2xl p-6 w-full max-w-sm flex flex-col items-center">
          <h1 className="text-xl md:text-2xl font-bold mb-3 text-[#ffffff] text-center" style={{ color: "#CD3A38" }}>
            Enter Verification Code
          </h1>
          <p className="text-gray-100 text-sm md:text-base text-center mb-4" style={{ color: "#295E7E" }}>
            ...
            {otp.map((digit, index) => (
              <input
                ...
                className="w-9 h-9 md:w-11 md:h-11 text-center text-lg font-semibold border-2 border-[#ffffff] rounded-lg 
                           focus:border-[#CD3A38] focus:outline-none transition-all bg-white shadow-sm" style={{ color: "#295E7E" }}
